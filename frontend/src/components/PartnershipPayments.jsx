...
export default function PlotPurchase() {
  ...
  const [activeSection, setActiveSection] = useState('payments'); // 'payments' | 'agents'
  ...
  const [agentForm, setAgentForm] = useState({
    agentName: '', commissionAmount: '', paidAmount: '0',
  ...
  const getPartnerStats = (plot) => plot.partners.map(partner => {
    const paid = plot.payments.filter(p => p.paidBy === partner.name).reduce((s, p) => s + p.amount, 0);
    return { ...partner, paid, pending: partner.shareAmount - paid, paidPercent: Math.min(100, Math.round((paid / partner.shareAmount) * 100)) };
  });
  ...
  const handleAddPayment = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/plots/${selectedPlot._id}/payments`, fd, {
  ...
  const handleAddAgent = async (e) => {
    ...
    try {
      ...
      await axios.post(`${API_URL}/plots/${selectedPlot._id}/agents`, fd, {
        headers: { 'Content-Type': 'multipart/form-data' }
      });
      await fetchPlots();
      setShowAddAgent(false);
      setAgentFile(null);
      setAgentForm({ agentName: '', commissionAmount: '', paidAmount: '0', paymentDate: '', transactionId: '', paymentMode: 'Bank Transfer', proofUrl: '', notes: '' });
    } catch (err) {
  ...
  const proofLink = (payment) => {
    if (payment.proofFile) return `${SERVER_URL}${payment.proofFile}`;
    ...
    <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '1.5rem', color: 'var(--text-main)', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
        <div>
          ...
          <p style={{ margin: '0.25rem 0 0', color: 'var(--text-muted)', fontSize: '0.875rem' }}>Track joint land purchase — partner payments &amp; commission agents</p>
        </div>
      ...
      {selectedPlot && (() => {
        ...
        const totalCommPaid = (selectedPlot.commissionAgents || []).reduce((s, a) => s + a.paidAmount, 0);
        ...
        return (
          <>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', gap: '2rem', textAlign: 'right', flexWrap: 'wrap' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '1.5rem', fontWeight: 700, color: totalCommPaid < totalCommission ? '#b91c1c' : '#15803d' }}>{fmt(totalCommission)}</div>
                  </div>
                </div>
              </div>
                ...
                <div key={i} style={{ background: 'var(--bg-card)', borderRadius: '12px', border: `2px solid ${ps.pending <= 0 ? '#22c55e' : 'var(--border-color)'}`, padding: '1.25rem' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '0.5rem', marginBottom: '1rem' }}>
                    {[
                      ...
                      <div key={label} style={{ textAlign: 'center', background: 'rgba(0,0,0,0.03)', borderRadius: '8px', padding: '0.6rem 0.4rem' }}>
                    ...
                    <div style={{ marginTop: '0.75rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                      {selectedPlot.payments.filter(p => p.paidBy === ps.name).sort((a, b) => new Date(b.date) - new Date(a.date)).map(payment => (
                        <div key={payment._id} style={{ background: 'rgba(0,0,0,0.03)', borderRadius: '8px', padding: '0.75rem', fontSize: '0.8rem', position: 'relative' }}>
                          ...
                          <button onClick={() => handleDeletePayment(payment._id)} style={{ position: 'absolute', top: '0.5rem', right: '0.5rem', background: 'none', border: 'none', cursor: 'pointer', color: '#b91c1c', padding: '0.2rem' }}>
                            <Trash2 size={13} />
                          </button>
                        </div>
                      ))}
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem' }}>
              {[
                { key: 'payments', label: '💰 All Payments', count: selectedPlot.payments.length },
                        ...
                        {[...selectedPlot.payments].sort((a, b) => new Date(b.date) - new Date(a.date)).map(payment => (
                          <tr key={payment._id} style={{ borderBottom: '1px solid var(--border-color)' }}>
                            ...
                            <td style={{ padding: '0.75rem' }}>
                              {proofLink(payment)
                                ? <a href={proofLink(payment)} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent-primary)', display: 'flex', alignItems: 'center', gap: '0.2rem', whiteSpace: 'nowrap' }}><Eye size={14} /> View</a>
                                : '—'}
                            </td>
                            <td style={{ padding: '0.75rem' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                  <div>
                    ...
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
                      Total: {fmt(totalCommission)} | Paid: <span style={{ color: '#15803d' }}>{fmt(totalCommPaid)}</span> | Pending: <span style={{ color: '#b91c1c' }}>{fmt(totalCommission - totalCommPaid)}</span>
                    </div>
                  </div>
                ...
                {(selectedPlot.commissionAgents || []).length === 0 ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    {selectedPlot.commissionAgents.map(agent => {
                      ...
                      const agentProof = agent.proofFile ? `${SERVER_URL}${agent.proofFile}` : agent.proofUrl || null;
                      return (
                        <div key={agent._id} style={{ border: `1px solid ${pending <= 0 ? '#22c55e' : 'var(--border-color)'}`, borderRadius: '10px', padding: '1rem', position: 'relative' }}>
                          ...
                          <button onClick={() => handleDeleteAgent(agent._id)}
                            style={{ position: 'absolute', top: '0.75rem', right: '0.75rem', background: 'none', border: 'none', cursor: 'pointer', color: '#b91c1c' }}>
                            <Trash2 size={14} />
                          </button>
                        </div>
                      );
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px rgba(0,0,0,0.3)' }}>
            ...
            <form onSubmit={handleAddPayment}>
              ...
              <FileUploadField label="Upload Proof (Receipt / Screenshot / PDF)" value={paymentFile} onChange={setPaymentFile} />
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', padding: '2rem', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 50px rgba(0,0,0,0.3)' }}>
            ...
            <form onSubmit={handleAddAgent}>
              ...
              <FileUploadField label="Upload Proof (Receipt / Screenshot / PDF)" value={agentFile} onChange={setAgentFile} />
